...
onMounted(() => {
  viableJobs.map((x) =>
    jobsStore.viableJobs.push({ title: x.Titel, job: null, details: null })
  );
  nonviableJobs.map((x) =>
    jobsStore.nonviableJobs.push({ title: x.Titel, job: null, details: null })
  );
...
export default {
  ...
  methods: {
    ...
    weighted() {
      ...
      this.jobsStore.doneJobs.forEach(
        (job) =>
          (job.weight = this.getWeight(
            job.monthStart,
            job.yearStart,
            job.monthEnd,
            job.yearEnd
    ...
    removeJob() {
      this.jobsStore.doneJobs = this.arrayRemove(
        this.jobsStore.doneJobs,
        this.jobsStore.doneJobs[this.jobsStore.doneJobs.length - 1]
      );
    },
    getJobs() {
      const body1 = [
      ...
      const jobsArray = this.jobsStore.doneJobs.map((job) => [
        job.id,
        job.title,
        job.monthStart,
        job.yearStart,
      ...
      const docDefinition = {
        header: {
          text: "CV 2.0: Interactief CV gebaseerd op de CompetentNL standaard",
          margin: [20, 20, 10, 20],
        },
        footer: {
          columns: [
  ...
  watch: {
    data() {
      ...
      this.data.map((x) =>
        this.jobsStore.allJobs.push({
          title: x.title,
          job: x.link.split("/api/")[1],
          details: null,
        })
      );
  ...
  computed: {
    filteredData() {
      ...
      const sortedItems = this.sortedData.filter((job) =>
        job.title.startsWith(
    ...
    sortedData() {
      ...
      const sortedData = this.jobsStore.allJobs.sort((a, b) =>
  ...
  <n-config-provider :theme="darkTheme">
    <n-table :bordered="false" :single-line="false">
        ...
        <tr>
          <th>
            <n-input
              ...
              placeholder="voer eerste letters beroep in"
            >
              <template #prefix>
      ...
      <tbody>
        ...
        <tr>
          <td>
            <n-button strong secondary type="primary" @click="appendJob">
              Voeg toe
            </n-button>
          </td>
          <td>
            <n-button strong type="secondary" @click="removeJob">
              Verwijder
            </n-button>
          </td>
          <td></td>
    ...
    <n-card label="CV 2.0" content-style="padding: 20;">
      ...
      >
        <n-tab-pane
          v-for="job in jobsStore.doneJobs"
          :key="job.id"
          :name="job.title"
        >
          {{ job.id }}: {{ job.title }} | {{ job.monthStart }}-{{
            job.yearStart
          }}
          tot {{ job.monthEnd }}-{{ job.yearEnd }} | gewicht: {{ job.weight }}
        </n-tab-pane>
      </n-tabs>
      <template #action>
